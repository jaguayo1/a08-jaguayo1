...
public class JSpot extends JPanel implements MouseListener, Spot {
	...
	//It needs to be observable to listeners. We need a list to update.
	...
	public JSpot(Color background, Color spot_color, Color highlight, 
		...
		//It already has a method to set and get the color. We just call it here.
		...
		//We add ourselves as a mouseListener to ourselves.
	...
	public void setSpot() {
		_is_empty = false;
		trigger_update(); //I need to force an update every time I make a change. JPanel doesn't know to do it on its own.
	}
	...
	public void paintComponent(Graphics g) {
		...
		//turn virtualness off an use jPanels paintComponent.
		super.paintComponent(g);

		Graphics2D g2d = (Graphics2D) g.create(); //Creates and returns a copy of itself.
		if (isHighlighted()) {
			g2d.setColor(getHighlight());
			g2d.setStroke(new BasicStroke(4));
			g2d.drawRect(0, 0, getWidth(), getHeight()); //inherited properties of JPanel.
		}
		if (!isEmpty()) {
	...
	@Override
	public void mousePressed(MouseEvent e) { //These are all part of the contract. Need to implement even if we don't use.
	...
	private void trigger_update() {		
		...
		new Thread(new Runnable() { //Burning time here so we can recall repaint and have it work.
			public void run() {
				try {
					Thread.sleep(50);
